...
/// 生成 mock 数据：根据全局行号 row_index 确定性生成一条记录。
/// 用于向 partitioned_executed_trades 插入约 1.12 亿条数据，满足各分区 >1000 万且 2024-07-22 额外 200 万。
#[cfg(test)]
...
/// 根据 row_index 确定所属分区及 block_time（确定性）。
/// 分区 0..=10 对应 before_y2024 到 y2026q01；分区 3 内前 200 万行落在 2024-07-22。
#[cfg(test)]
...
mod tests {
    ...
    /// 向 partitioned_executed_trades 插入约 1.12 亿条确定性 mock 数据。
    /// 分区 executed_trades_before_y2024 到 executed_trades_y2026q01 各超过 1000 万条；
    /// 2024-07-22 当天额外 200 万条。不向 y2026q02、y2026q03 插入。
    #[ignore]
    #[test]

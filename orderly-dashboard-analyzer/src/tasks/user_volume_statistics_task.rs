...
use crate::{
    ...
    db::{
        hourly_user_perp::{
            get_single_user_trading_volume_in_time_range, get_user_trading_volume_in_time_range,
        },
...
pub async fn cal_user_volume_statistics(base_url: &str) -> anyhow::Result<()> {
    ...
    loop {
        ...
        // ytd
        let ytd_res1 =
            single_get_user_trading_volume_in_time_range(&account_ids, ytd_from, ytd_to, "ytd")
                .await?;

        // 90d
        let account_ids = ytd_res1.keys().cloned().collect();
        let d90_res1 =
            single_get_user_trading_volume_in_time_range(&account_ids, d90_from, d90_to, "d90")
                .await?;

        // 30d
        let account_ids = d90_res1.keys().cloned().collect();
        let d30_res1 =
            single_get_user_trading_volume_in_time_range(&account_ids, d30_from, d30_to, "d30")
                .await?;

        // 7d
        let account_ids = d30_res1.keys().cloned().collect();
        let d7_res1 =
            single_get_user_trading_volume_in_time_range(&account_ids, d7_from, d7_to, "d7")
                .await?;

        // 1d
        let account_ids = d7_res1.keys().cloned().collect();
        let d1_res1 =
            single_get_user_trading_volume_in_time_range(&account_ids, d1_from, d1_to, "d1")
                .await?;
...
#[allow(dead_code)]
async fn single_get_user_trading_volume_in_time_range(
    account_ids: &Vec<String>,
    from_time: i64,
    to_time: i64,
    context: &str,
    ...
    for account_id in account_ids {
        let acc =
            get_single_user_trading_volume_in_time_range(account_id.clone(), from_time, to_time)
                .await?;
        if let Some(volume) = acc.volume {
            cache.insert(account_id.clone(), volume);
        }

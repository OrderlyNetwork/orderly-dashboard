...
pub async fn analyzer_perp_trade(
    ...
    for perp_trade in trades {
        ...
        // hourly_orderly
        let perp_key = HourlyOrderlyPerpKey {
        ...
        match hourly_user_perp_op {
            None => {
                ...
                let saved_data = find_hourly_user_perp(
                    perp_key_clone.account_id,
                    perp_key_clone.symbol,
                    block_hour.clone(),
                )
                .await
                .unwrap();
        ...
        match user_perp_summary_op {
            None => {
                let saved_summary = find_user_perp_summary(
                    user_perp_summary_key.account_id.clone(),
                    perp_symbol.clone(),
                )
                .await
                .unwrap();
    ...
    create_or_update_orderly_perp_summary(Vec::from_iter(orderly_summary_map.values().into_iter()))
        .await
        .unwrap();
    create_or_update_user_perp_summary(Vec::from_iter(user_summary_map.values().into_iter()))
        .await
        .unwrap();

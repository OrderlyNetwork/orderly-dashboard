        ...
        loop {
            ...
            match response_str {
                Ok(json_str) => {
                    ...
                    to_time = cal_to_time(
                        from_time + empty_counter * (batch_block_num as i64),
                        batch_block_num,
                    );
                    ...
                    create_or_update_block_summary(block_summary.clone())
                        .await
                        .ok();
                    tracing::info!(target: ANALYZER_CONTEXT,"ok to pull block from time: {}, to_time: {},from {} to {}. cost:{}", from_time, to_time,round_from_block,round_to_block,Utc::now().timestamp_millis()-timestamp);
                }
                Err(error) => {
        ...
        match get_block_timestamp(&base_url, block_num).await {
            Ok(data) => match data {
                Response::Success(data) => {
                    return data.into_data().unwrap().block_timestamp;
                }
                Response::Failure(err) => {
                    tracing::info!("get_block_timestamp response failed with err: {:?}", err);
...
pub async fn get_block_timestamp(
    ...
    let indexer_url = format!("{}/pull_block_timestamp?block_number={}", base_url, block_num,);
    let response = reqwest::get(indexer_url).await?;

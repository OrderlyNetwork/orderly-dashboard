...
async fn handle_settlement(
    ...
    tracing::info!(target:SETTLEMENT_ANALYZER,"handle_settlement, account_id: {}, after user_perp_snap.cost_position:{:?}, settlement.settled_amount:{:?}",user_perp_snap.account_id,user_perp_snap.cost_position.to_string(),settlement.settled_amount.to_string());
}

...
/**
 * SiteFooter — standalone Orderly Network footer component.
 *
 * Dependencies to copy to a new project:
 *   - src/app/shared/orderly.ts        (nav data)
 *   - src/imports/IcSocialYoutubeS24.tsx
 *   - src/imports/UniswapLogo.tsx
 *   - src/imports/CamelotLogo.tsx
 *   - public/images/svg/  (EthBadge.svg, ArbBadge.svg, SolBadge.svg, PancakeLogo.svg, WOOFiLogo.svg)
 *   - public/images/logos/raydium.png
 *   - npm install motion
 */
...
import IcSocialYoutubeS from "@/imports/IcSocialYoutubeS24";
import UniswapLogo from "@/imports/UniswapLogo";
import CamelotLogo from "@/imports/CamelotLogo";
import { FOOTER_NAV, type FooterNavLink } from "@/app/shared/orderly";

// ─── Inlined SVG paths ────────────────────────────────────────────────────────
const SVG = {
  // Orderly brandmark (O icon) — 4 pieces
  brandmark1:
...
function EthBadge() {
  return <img src="/images/svg/EthBadge.svg" width={22} height={22} alt="Ethereum" className="shrink-0 rounded-[6px]" />;
}
function ArbBadge() {
  return <img src="/images/svg/ArbBadge.svg" width={22} height={22} alt="Arbitrum" className="shrink-0 rounded-[6px]" />;
}
function SolBadge() {
  return <img src="/images/svg/SolBadge.svg" width={22} height={22} alt="Solana" className="shrink-0 rounded-[6px]" />;
}
function PancakeSwapLogo() {
  return <img src="/images/svg/PancakeLogo.svg" width={44} height={44} alt="PancakeSwap" className="shrink-0 rounded-full" />;
}
function RadyiumLogo() {
  return <img src="/images/logos/raydium.png" width={44} height={44} alt="Raydium" className="shrink-0 rounded-full" />;
}
function WoofLogo() {
  return <img src="/images/svg/WOOFiLogo.svg" width={44} height={44} alt="WOOFi" className="shrink-0 rounded-full" />;
}
...
function OrderPriceDisplay({ onBuyOrder }: { onBuyOrder?: () => void }) {
  ...
  return (
    <div className="flex items-center gap-[16px] relative shrink-0">
      {price && (
        <div
          className="flex flex-col font-['Atyp_BL:Semibold',sans-serif] h-[38px] justify-center not-italic relative text-[24px] text-white tracking-[0.24px]"
...
// ─── Brandmark (large O icon in footer) ──────────────────────────────────────
function Brandmark() {
  return (
...
// ─── Footer left panel: brandmark + ORDER price + "Buy ORDER" button ──────────
...
function FooterLinkItem({ label, href, external, target }: FooterNavLink) {
  ...
  const linkProps = opensNewTab ? { target: "_blank" as const, rel: "noopener noreferrer" } : {};
  const LinkComponent = opensNewTab ? "a" : Link;

  return (
    <LinkComponent href={href} {...linkProps} className="no-underline hover:opacity-80 transition-opacity">
...
// ─── "All systems operational" pill + social icons ───────────────────────────
    ...
    <footer style={{ background: "#000", paddingTop: 60 }}>
      ...
      <div style={{ maxWidth: 1302, margin: "0 auto", padding: "0 24px 24px" }}>
        <div style={{ background: "#6700ce", borderRadius: 30, overflow: "hidden" }}>
          {/* Top row: ORDER price/CTA + nav link columns */}
          ...
          {/* Bottom bar: status + social icons */}

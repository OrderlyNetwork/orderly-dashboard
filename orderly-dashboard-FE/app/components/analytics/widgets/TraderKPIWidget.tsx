...
export const TraderKPIWidget: FC<Props> = ({ data }) => {
  const { volumeRows, accountRows, marketRows } = data;
  ...
  return (
    <div className="dash-grid-sm">
      ...
      <KPICard flat label="30D Volume" value={fmtCompact(vol30d)} delta={fmtDeltaPct(vol30d, vol30dPrev)} subValue="rolling 30-day total" />
      ...
      <KPICard flat label="Total Accounts" value={totalAccounts >= 1e6 ? `${(totalAccounts / 1e6).toFixed(2)}M` : totalAccounts.toLocaleString()} delta={fmtDeltaPct(totalAccounts, prevWeekAccounts)} subValue="vs last week" />
    </div>
  );
};

...
export const LiquidationsBySymbolWidget: FC = () => {
  const { data, isLoading, error } = usePerpLiquidations();
  ...
  rows.forEach((r) => {
    if (!r.liquidation_notional_usd || r.liquidation_notional_usd <= 0) return;
    const key = r.date;
    const symbol = r.symbol;
    allKeys.add(symbol);
    if (!dateMap.has(key)) dateMap.set(key, {});
    dateMap.get(key)![symbol] = r.liquidation_notional_usd;
  });
